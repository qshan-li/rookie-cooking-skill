...
验证 `rookie-cooking-skill` 在不同 agent 终端中执行 Recipe Generation 时，能按 `SKILL.md` 的 Interactive QA Mode 规则工作。测试以自动化 harness 为主，人工观察为辅：

- 用户没有指定输出模式，且终端支持交互选择工具时，先让用户在默认和厨房执行版之间选择。
- 用户已经指定输出模式时，不再重复询问。
- 当前终端没有交互选择能力时，不阻塞生成，直接使用默认输出模式。
- Troubleshooting、Learning、Meal Planning、Recipe Import 不误触发 Recipe Generation 的输出模式 QA。
...
- Claude Code project skill：`.claude/skills/rookie-cooking-skill/SKILL.md`。
- Claude Code personal skill：`~/.claude/skills/rookie-cooking-skill/SKILL.md`。
...
| 编号 | 场景 | Prompt | 预期行为 |
| --- | --- | --- | --- |
| A | Recipe Generation，未指定输出模式 | `Use $rookie-cooking-skill 生成番茄炒蛋。` | 支持交互选择的 agent 先提供默认和厨房执行版两个选项；不支持时直接默认生成。 |
| B | Recipe Generation，明确厨房版 | `Use $rookie-cooking-skill 生成番茄炒蛋，只要厨房版。` | 不触发 QA，直接输出厨房执行版。 |
| C | Recipe Generation，明确默认 | `Use $rookie-cooking-skill 生成 2 人份青椒肉丝，选择默认。` | 不触发 QA，输出完整解释版，并询问 PDF 或打印。 |
| D | Recipe Generation，明确厨房执行版 | `Use $rookie-cooking-skill 生成 4 人份红烧肉，家里是电磁炉，选择厨房执行版。` | 不触发 QA，输出厨房执行版，并询问 PDF 或打印。 |
| E | Troubleshooting | `Use $rookie-cooking-skill 诊断：我做的蒸蛋有很多蜂窝，表面还出水。` | 不触发输出模式 QA，直接给失败诊断、原因、风险和下次调整。 |
| F | Learning | `Use $rookie-cooking-skill 为什么炒青菜会出水？` | 不触发输出模式 QA，输出原理解释并关联可练习菜。 |
| G | Meal Planning | `Use $rookie-cooking-skill 两菜一汤给 3 个人，怎么安排？` | 不触发输出模式 QA，输出菜单、购物清单、时间线和设备冲突。 |
| H | Recipe Import | `Use $rookie-cooking-skill 把这个菜谱改写成新手版：鸡蛋两个，番茄两个，炒熟即可。` | 不触发输出模式 QA，进入导入改写流程，初始 review 状态为 `draft`。 |
...
| 厨房执行版 | 厨房执行版、安全提示、失败信号、交互式后续交付选择 | 完整解释版长篇说明 |
...
| Agent | 安装发现 | ACP check | Headless A | B 厨房版 | C 默认 | D 厨房执行版 | E 诊断 | F 学习 | G 一餐规划 | H 导入 | 结论 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Codex |  | acpx |  |  |  |  |  |  |  |  |  |
| Claude Code |  | acpx adapter |  |  |  |  |  |  |  |  |  |
| Gemini CLI |  | acpx/native |  |  |  |  |  |  |  |  |  |
| Hermes Agent |  | acpx raw agent |  |  |  |  |  |  |  |  |  |
| OpenClaw | 可选 | orchestrator | 可选 | 可选 | 可选 | 可选 | 可选 | 可选 | 可选 | 可选 | 可选 |
...
- P1：Recipe Generation 输出缺少关键安全提示或失败信号。
